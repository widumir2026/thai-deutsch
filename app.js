...
function tUI(key){
  const de = {
    ...
    hint_tap_flip: "Tippen zum Umdrehen",
    flip: "Umdrehen",
    again: "Nochmal",
    hard: "Schwer",
    good: "Gekonnt",
    next: "Nächste",
    audio_front: "Audio vorne",
    audio_back: "Audio hinten",
    reset: "Fortschritt löschen",
    csv_title: "Wortliste (CSV)",
    csv_desc: "Import: Header th,roman,en,de,cat. UTF-8. Kommas in Feldern vermeiden.",
    choose_file: "Datei auswählen",
    export_csv: "Export CSV",
    clear_custom: "Eigenes Set löschen",
    active: "Aktiv:",
    pwa_title: "Installieren (PWA)",
    pwa_desc: "iPhone: Safari -> Teilen -> \"Zum Home-Bildschirm\". Android/Chrome: \"Installieren\". Offline nach erstem Laden.",
    status: "Status:",
    version: "Version:",
    wrong_title: "Multiple-Choice falsch",
    wrong_desc: "Hier siehst du deine falschen Antworten (pro Deck). Du kannst daraus ein Lernset machen.",
    wrong_learn: "Nur falsche lernen",
    wrong_learn_mc: "Falsche als Multiple-Choice",
    wrong_clear: "Falsche Liste leeren",
    back_normal: "Zurück zum normalen Lernen",
    no_wrong: "Noch keine falschen Antworten.",
    mc_rf: " | Multiple-Choice richtig/falsch: "
  };
  const th = {
    title: "การ์ดคำศัพท์ไทย",
    hint_tap_flip: "แตะเพื่อพลิก",
    flip: "พลิก",
    again: "อีกครั้ง",
    hard: "ยาก",
    good: "จำได้",
    next: "ถัดไป",
    audio_front: "เสียงด้านหน้า",
    audio_back: "เสียงด้านหลัง",
    reset: "ลบความคืบหน้า",
    csv_title: "รายการคำ (CSV)",
    csv_desc: "นำเข้า: ต้องมีหัวตาราง th,roman,en,de,cat (UTF-8). หลีกเลี่ยงเครื่องหมายจุลภาคในช่องข้อมูล.",
    choose_file: "เลือกไฟล์",
    export_csv: "ส่งออก CSV",
    clear_custom: "ลบชุดของฉัน",
    active: "ชุดที่ใช้อยู่:",
    pwa_title: "ติดตั้ง (PWA)",
    pwa_desc: "iPhone: Safari -> แชร์ -> \"เพิ่มไปยังหน้าจอโฮม\". Android/Chrome: \"ติดตั้ง\". ใช้งานออฟไลน์หลังโหลดครั้งแรก.",
    status: "สถานะ:",
    version: "เวอร์ชัน:",
    wrong_title: "ปรนัยที่ตอบผิด",
    wrong_desc: "ดูข้อที่ตอบผิด (แยกตามชุด) และทำเป็นชุดทบทวนได้",
    wrong_learn: "เรียนเฉพาะที่ตอบผิด",
    wrong_learn_mc: "ฝึกแบบปรนัย (เฉพาะผิด)",
    wrong_clear: "ลบรายการผิด",
    back_normal: "กลับไปเรียนปกติ",
    no_wrong: "ยังไม่มีข้อที่ตอบผิด",
    mc_rf: " | ปรนัย ถูก/ผิด: "
  };
  const dict = (uiLang==="th") ? th : de;
  return dict[key] || de[key] || key;
}

function updateUIStrings(){
  if(el.titlePill) el.titlePill.textContent = tUI("title");
  if(el.hint) el.hint.textContent = tUI("hint_tap_flip");

  if(el.flip) el.flip.textContent = tUI("flip");
  if(el.again) el.again.textContent = tUI("again");
  if(el.hard) el.hard.textContent = tUI("hard");
  if(el.good) el.good.textContent = tUI("good");
  if(el.next) el.next.textContent = tUI("next");
  if(el.speakFront) el.speakFront.textContent = tUI("audio_front");
  if(el.speakBack) el.speakBack.textContent = tUI("audio_back");
  if(el.resetProgress) el.resetProgress.textContent = tUI("reset");

  if(el.csvTitle) el.csvTitle.textContent = tUI("csv_title");
  if(el.csvDesc) el.csvDesc.textContent = tUI("csv_desc");
  if(el.chooseCsv) el.chooseCsv.textContent = tUI("choose_file");
  if(el.exportCsv) el.exportCsv.textContent = tUI("export_csv");
  if(el.clearCustom) el.clearCustom.textContent = tUI("clear_custom");
  if(el.activeLabel) el.activeLabel.textContent = tUI("active");

  if(el.pwaTitle) el.pwaTitle.textContent = tUI("pwa_title");
  if(el.pwaDesc) el.pwaDesc.textContent = tUI("pwa_desc");
  if(el.statusLabel) el.statusLabel.textContent = tUI("status");
  if(el.versionLabel) el.versionLabel.textContent = tUI("version");

  if(el.wrongTitle) el.wrongTitle.textContent = tUI("wrong_title");
  if(el.wrongDesc) el.wrongDesc.textContent = tUI("wrong_desc");
  // wrong buttons are updated in updateWrongPanel as well
}
...
function updateWrongPanel(){
  ...
  el.learnWrong.textContent = studyWrongOnly ? tUI("back_normal") : tUI("wrong_learn");
  if(el.learnWrongMC){ el.learnWrongMC.textContent = tUI("wrong_learn_mc"); }
  el.clearWrong.textContent = tUI("wrong_clear");

  if(unique.length===0){
    el.wrongList.innerHTML = `<div class="small">${tUI("no_wrong")}</div>`;
    return;
  }
...
function updateTop(){
  ...
  const mcPart = tUI("mc_rf") + s.correct + " / " + wrong;
...
// Custom CSV chooser button (translateable)
el.chooseCsv?.addEventListener("click", ()=>{
  el.csvFile.click();
});
function setCsvFileName(name){
  if(!el.csvFileName) return;
  el.csvFileName.textContent = name ? ("(" + name + ")") : "";
}
setCsvFileName("");
